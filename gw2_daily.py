...
def get_daily_info(*styles, show_all=False):
	...
	assert len(quest_name) == len(id_list['IDs']), "Warning: Lists of ID and name are not the same length. len(ID): "+str(len(id_list['IDs']))+"\t len(Name): "+str(len(quest_name))
